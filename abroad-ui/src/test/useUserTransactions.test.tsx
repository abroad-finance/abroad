...
import type { TransactionData } from '../services/public/transactionTypes'
...
describe('useUserTransactions', () => {
  ...
  describe('fetchTransactions', () => {
    ...
    it('should pass options to getUserTransactions', async () => {
      vi.mocked(getUserTransactions).mockResolvedValue({
        data: {
          page: 1, pageSize: 10, total: 0, transactions: [],
        },
        headers: new Headers(),
  ...
  describe('fetchAllTransactions', () => {
    ...
    it('should always fetch with confirmedOnly: false and pageSize: 100', async () => {
      vi.mocked(getUserTransactions).mockResolvedValue({
        data: {
          page: 1, pageSize: 100, total: 0, transactions: [],
        },
        headers: new Headers(),
  ...
  describe('filteredTransactions', () => {
    it('should filter transactions by chain when selectedChainKey provided', async () => {
      const celoAndStellarTransactions = [{
        ...
        quote: { ...mockTransactions[0].quote, network: 'CELO' as const },
      }, {
        ...mockTransactions[1], // STELLAR
        quote: { ...mockTransactions[1].quote, network: 'STELLAR' as const },
      }]
    ...
    it('should not filter when selectedChainKey is undefined', async () => {
      vi.mocked(getUserTransactions).mockResolvedValue({
        data: {
          page: 1, pageSize: 20, total: 2, transactions: mockTransactions,
        },
        headers: new Headers(),
      ...
      vi.mocked(getUserTransactions).mockResolvedValue({
        data: {
          ...
          transactions: [
            ...
            { ...mockTransactions[0], createdAt: '2024-01-15T12:00:00Z', id: 'tx-003' },
          ],
        },
        headers: new Headers(),
  ...
  describe('allTransactions mapping', () => {
    ...
    it('should map transaction status correctly', async () => {
      const transactionsWithDifferentStatus = [
        ...
        { ...mockTransactions[0], id: 'tx-002', status: 'PAYMENT_COMPLETED' as const },
        { ...mockTransactions[0], id: 'tx-003', status: 'PAYMENT_EXPIRED' as const },
      ]
  ...
  describe('date formatting', () => {
    it('should format recent dates correctly', async () => {
      ...
      expect(result.current.recentTransactions[0].time).toMatch(/5\s*(m|minutes?)\s*ago/)
    })
    ...
    it('should format older dates correctly', async () => {
      ...
      expect(result.current.recentTransactions[0].time).toMatch(/5\s*(h|hours?)\s*ago/)
    })
  })
})
