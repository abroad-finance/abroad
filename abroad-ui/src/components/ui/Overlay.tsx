...
export const Overlay: React.FC<OverlayProps> = ({ children, className, onClose }) => {
  ...
  return (
    <div
      aria-label={t('overlay.close', 'Close overlay')}
      className={cn(
      ...
      tabIndex={0}
    >
      <dialog className="relative border-0 p-0 m-0 bg-transparent" onClick={e => e.stopPropagation()} onKeyDown={e => e.stopPropagation()} open role="dialog">
        {children}
      </dialog>
    </div>
  )
